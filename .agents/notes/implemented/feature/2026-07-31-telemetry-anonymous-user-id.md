...
| Home | `@deepseek-ai/dsh-anonymous-user-id`, shared by the OTel backend, `/feedback`, and direct Krokki requests | Consumers share one storage contract without depending on an exporter backend |
| Separate switch | None | Any consumer can create the identity; `DSH_TELEMETRY_DISABLED` stops telemetry reporting but does not disable feedback acknowledgement or the Krokki request header |
...
- The OTel feed, `/feedback`, and direct Krokki requests share `.anonymous-user-id`.
